...
# review variables containing area information
ds_train[, ..sf_vars]

# calculate total area
ds_train[, total_area := round((
  LotArea +
    MasVnrArea +
    TotalBsmtSF +
    GarageArea +
...
```{r id}
# pool has many NAs, but I do not remove it because it should be useful in indicating if a property may have a high value
ds_full$PoolQC[is.na(ds_full$PoolQC)] <- 'None'
Qualities <- c('None' = 0, 'Po' = 1, 'Fa' = 2, 'TA' = 3, 'Gd' = 4, 'Ex' = 5)
ds_full$PoolQC <- as.integer(plyr::revalue(ds_full$PoolQC, Qualities))
ds_full[, table(PoolQC, useNA = "always")]
# determine which variables should be kept, nr. of NAs low
var_na <- ds_full[, colMeans(is.na(ds_full)) < .95]
...
# show number of unique values in charater vars; which we will convert to factors in next steps
apply(ds_full_char, 2, function(x) length(unique(x)))

remove(ds_full_char)

#count unique values for each variable in ds_full
...
conv <- sapply(ds_full, function(x) is.character(x) && length(unique(x)) < 30)
...
# Cleaning NAs
```{r}
# TODO replace NAs with zeros with lapply; 

ds_full <- as.data.frame(ds_full)
ds_full[is.na(ds_full)] <- 0
...
```{r}
set.seed(96)
preproc_alg <- preProcess(ds_train, method = c("BoxCox", 
                                               "center", 
                                               "scale"
                                               # "pca"
                                               ))
...
min_rmse_lm <- min(model_lm_caret$results$RMSE)
```
...
min_rmse_svm <- min(model_svm_caret$results$RMSE)
```
...
min_rmse_rf <- min(model_rf_caret$results$RMSE)
```
